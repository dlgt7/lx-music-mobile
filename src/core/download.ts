  ...
  if (settingState.setting['download.writePicture']) {
    try {
      await retryDownloadOption('封面写入', async () => {
        let picPath = '';
        try {
          const picUrl = await getPicUrl({ musicInfo: task.musicInfo, isRefresh: false });
...
export const addTask = (
  ...
  const downloadDir = target === 'onedrive'
    ...
    : settingState.setting['download.path'] || (RNFetchBlob.fs.dirs.MusicDir + '/LX Music');
...
export const getDownloadList = () => downloadState.tasks;

export const initDownloadList = async () => {

  const tasks = await getDownloadTasks();
  downloadActions.setTasks(tasks);
  return tasks;
};